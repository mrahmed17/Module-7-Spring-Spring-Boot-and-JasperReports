...
export class SalaryService {
  ...
  updateSalary(salaryId: number, salary: SalaryModel): Observable<SalaryModel> {
    return this.http.put<SalaryModel>(`${this.baseUrl}/update/${salaryId}`, salary);
  }
  ...
  getSalariesByUserAndYear(userId: number, year: number): Observable<SalaryModel[]> {
  ...
  getLatestSalaryByUser(userId: number): Observable<SalaryModel[]> {
    return this.http.get<SalaryModel[]>(`${this.baseUrl}/user/${userId}/latest`);
  }
  ...
  getSalariesByDateRange(startDate: Date, endDate: Date): Observable<SalaryModel[]> {
