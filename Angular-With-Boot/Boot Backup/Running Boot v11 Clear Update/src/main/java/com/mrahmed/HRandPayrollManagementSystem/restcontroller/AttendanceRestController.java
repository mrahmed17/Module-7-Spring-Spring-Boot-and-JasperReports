...
public class AttendanceRestController {
    ...
    public ResponseEntity<List<Attendance>> getOvertimeByUser(
            ...
            @RequestParam("startDate") @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDateTime startDateTime,
            @RequestParam("endDate") @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDateTime endDateTime) {

        List<Attendance> overtimeRecords = attendanceService.getOvertimeForUser(userId, startDateTime, endDateTime);
    ...
    @GetMapping("/overtimeInRange")
    public ResponseEntity<List<Attendance>> getOvertimeInRange(
            @RequestParam("startDate") @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate startDate,
            @RequestParam("endDate") @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate endDate) {
        List<Attendance> overtimeRecords = attendanceService.getOvertimeInRange(startDate, endDate);
        if (overtimeRecords.isEmpty()) {
            return ResponseEntity.noContent().build(); // Return 204 if no overtime found
        }
        return ResponseEntity.ok(overtimeRecords); // Return the list of overtime records
